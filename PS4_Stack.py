    ...
    def __init__(self, filename, debug=False):
        ...
        for line in fo:
            ...
            if root_vertex in self.__originalGraph.keys(): # Root vertex Entry Already Exists, update the existing entry
                self.__originalGraph[root_vertex].add_adjVertex(slave_vertex)
                if debug:
                    # pass
...
#                 self.__originalGraph[root_vertex] = MyVertex(root_vertex)
                self.__originalGraph.update({root_vertex:MyVertex(root_vertex)})
...
#                 self.__originalGraph[root_vertex] = MyVertex(root_vertex)
            ...
            if slave_vertex not in self.__originalGraph.keys():
                self.__originalGraph.update({slave_vertex:MyVertex(slave_vertex)})
                if debug:
                    # pass
            ...
            if slave_vertex in self.__reverseGraph.keys(): # slave vertex Entry Already Exists
                self.__reverseGraph[slave_vertex].add_adjVertex(root_vertex)
                if debug:
                    pass
                ...
                self.__reverseGraph.update({slave_vertex:MyVertex(slave_vertex)})
                self.__reverseGraph[slave_vertex].add_adjVertex(root_vertex)
                if(debug):
                    pass
            ...
            if root_vertex not in self.__reverseGraph.keys():
                self.__reverseGraph.update({root_vertex:MyVertex(root_vertex)})
                if(debug):
                    pass
        ...
        if reverse:
            ...
            while len(dfs_stack)>0:
                ...
                if debug:
                    ...
                    print(self.__reverseGraph[last_vertex_in_stack].get_adjVertices())

                temp_sequence.append(last_vertex_in_stack)
                if(debug):
                    print('vertex {} appended to the temporary sequence'.format(last_vertex_in_stack))
                for adjVertex in self.__reverseGraph[last_vertex_in_stack].get_adjVertices():
                    if not self.__reverseGraph[adjVertex].checkIfExplored():
                        self.__reverseGraph[adjVertex].setExplored()
                        if debug:
            ...
            # for adjVertex in self.__reverseGraph[vertex_id].get_adjVertices():
            #     if not self.__reverseGraph[adjVertex].checkIfExplored():


                    # self.dfs(adjVertex, leader, None, reverse, debug)
            ...
            while len(dfs_stack) > 0:
                ...
                if debug:
                    print('The adjecent vertices of {} are:'.format(last_vertex_in_stack))
                    print(self.__originalGraph[last_vertex_in_stack].get_adjVertices())

                for adjVertex in self.__originalGraph[last_vertex_in_stack].get_adjVertices():
                    if not self.__originalGraph[adjVertex].checkIfExplored():
                        self.__originalGraph[adjVertex].setExplored()
                        if debug:
                            print('Now vertex {} has been set as explored\n'.format(adjVertex))
                        if debug:
